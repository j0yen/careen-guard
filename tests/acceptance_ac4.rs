...
/// survey-small.sh returns only 500 MB reclaimable — not enough to reach 90%.
const TOTAL: u64 = 100 * 1024 * 1024 * 1024;
const FREE: u64 = 2 * 1024 * 1024 * 1024;

#[test]
fn acceptance_ac4_breach_unresolved_when_candidates_exhaust() {
    ...
    // Must not panic — AC4 requires BreachUnresolved (not panic, not silent exit)
    careen_guard::guard::run(&args).expect("guard run should return Ok(ExitCode)");
